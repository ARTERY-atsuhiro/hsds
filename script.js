...
function crossetteEffect(star) {
	...
	createParticleArc(startAngle, PI_2, 4, 0.5, (angle) => {
		Star.add(
			x + (px - width / 2) * 3.0, // さらに広く拡散
			y + (py - height / 2) * 3.0,
			COLOR.Gold, // 見えやすい色に変更
			Math.random() * Math.PI * 2,
			20.0 + Math.random() * 30.0, // 速度を強化
			40000 + Math.random() * 50000 // ★ 寿命を大幅に延ばす (最短 4000ms, 最長 6000ms)
		);
	});
}
...
function fallingLeavesEffect(star) {
	createBurst(7, (angle, speedMult) => {
		...
		newStar.sparkSpeed = 1.28;
		newStar.sparkLife = 4750;
...
class Shell {
	...
	burst(x, y) {
		// 30% の確率で文字の花火を表示
		if (Math.random() < 0.1) {
			textFireworkEffect(x, y, "CHOOSE\nYOUR\nLIFE FES");
			return; // 文字の花火を表示したら、通常の処理は実行しない
		}
...
function textFireworkEffect(baseX, baseY, text) {
    console.log(`Text firework triggered at (${baseX}, ${baseY}) with text: ${text}`);

    const width = 200; // 文字の表示幅
    const height = 100; // 文字の表示高さ
    const fontSize = 24;

    const pixels = getTextPixels(text, fontSize, width, height);

    pixels.forEach(({ x, y }) => {
        Star.add(
            baseX + (x - width / 2) * 2.0, // 文字の火花の配置範囲を広げる
            baseY + (y - height / 2) * 2.0,
            COLOR.Red, // 目立つ色
            Math.random() * Math.PI * 2,
            0.05 + Math.random() * 0.04, // 速度を強化
            4000 + Math.random() * 2000 // ★ 寿命を大幅に延ばす
        );
    });
}
...
function getTextPixels(text, fontSize, width, height) {
    ...
    for (let y = 0; y < height; y += 3) {
        for (let x = 0; x < width; x += 3) {
            ...
            if (imageData.data[index] > 128) { // 白い部分の座標を取得
                pixels.push({ x, y });
            }
        }
    }
...
function getTextPixels(text, fontSize, width, height) {
    ...
    lines.forEach((line, index) => {
        ctx.fillText(line, width / 2, height / 2 - (lines.length - 1) * lineHeight / 2 + index * lineHeight);
    });
